...
def run(
    repo_path: Path = typer.Option(
        ...,
        "--repo-path",
        help="Path to the repository checked out on the Runloop devbox.",
    ),
    repo_url: Optional[str] = typer.Option(
        None, "--repo-url", help="Original GitHub repository URL."
    ),
    branch_name: str = typer.Option(
        "runloop/dependency-updates", "--branch-name", help="Branch to push changes to."
    ),
    dry_run: bool = typer.Option(
        True, "--dry-run/--no-dry-run", help="Disable pushes for local testing."
    ),
    llm_model: Optional[str] = typer.Option(
        None, "--llm-model", help="Chat model for LangChain (defaults to gpt-5-mini)."
    ),
    ...
    repo_meta = RepoMetadata(
        repo_path=repo_path, repo_url=repo_url, default_branch="main"
    )
    ...
    console.print("[bold green]Completed dependency update workflow[/bold green]")
